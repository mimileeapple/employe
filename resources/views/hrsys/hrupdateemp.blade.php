...
<form action="{{route('employees.update',$data->empid)}}" method="post" name="form1">
    ...
    <table border="1" class="tbl" style="margin: auto">
        <tr>
            <td>工號</td><td><input type="text" name="empid" value="{{$data->empid}}" disabled></td>
        ...
        <tr> <td>生日</td><td><input type="date" name="birth" value="{{$data->birth}}"></td>
            <td>婚姻</td><td>
                <select name="marry">
        ...
        <tr><td>地址</td><td colspan="3" style="text-align: left;padding-left:15px;">
            <input type="text" style="width:400px;" name="adress" value="{{$data->adress}}"></td></tr>
        <tr><td>部門</td><td><select id="dep" name="dep">
        ...
        <tr><td>職務代理人</td><td> <select name="agentemp">
                    <option value="" {{$data->agentemp ==""? 'selected':''}}></option>
                    @foreach($emp_list as $v)
                    <option value="{{$v->empid}}" {{$data->agentemp ==$v->empid? 'selected':''}}>{{$v->name}}</option>
                @endforeach
                </select></td>
            <td>在職狀態</td>
