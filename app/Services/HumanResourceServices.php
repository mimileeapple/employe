...
class HumanResourceServices{
    ...
    private $towho;
    private $content;
    function emp_list($page){//分頁
       return empinfo::paginate($page);
    }
    function select_emp(){//抓全部
        return empinfo::all();
    }
    function selectemp($id){//抓個人單筆資料 沒數組 a->name
        return empinfo::where('empid','=',$id)->first();
    }
    function selectempary($id){//抓回會是單筆資料例如 A[1]->name
        return empinfo::where('empid','=',$id)->get();
    }
    function selectmyleave($id){//抓自己全部請假資料

        return leaveorder::where('empid','=',$id)->orderBy('orderdate', 'DESC')->get();
    }
    function selectmyleaveall(){//抓全部請假資料
        return leaveorder::all();
    }
    function isholiday($datestart,$dateend){//判斷是否為節日予補班日
...
or manage2id=$id) and (manage1empsign='N' or manage2empsign='N')");
    }
